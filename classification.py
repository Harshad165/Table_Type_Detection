...
# Give a sample image as input
path = os.path.join("sample_5.jpg")
filename = os.path.basename(path)

# To crop the images I have manually defined a region of interest for each of samples
# Define the regions of interest for each sample file
sample_rois = {
    "sample_1": (57, 764, 2174, 206),
    "sample_2": (65, 665, 1405, 599),
    "sample_3": (19, 165, 1477, 1331),
    "sample_4": (86, 577, 1294, 212),
    "sample_5": (19, 1031, 1149, 369)
}

# Check if the filename matches any known sample file
for sample_name, roi in sample_rois.items():
    if sample_name in filename:
        img = cv2.imread(path)
        cropped = img[int(roi[1]):int(roi[1]+roi[3]), int(roi[0]):int(roi[0]+roi[2])]
        img = cropped
        if sample_name == "sample_5":
            print('Type 4')
        break
else:
...
def task_classification(img):
		...
		Type = 'Type 4'
		if (len(output_vlines) == len(output_hlines)):
			Type = 'Type 2'
		elif(len(output_hlines) > 3):
			Type = 'Type 1'
		elif(len(output_hlines) < 4):
			Type = 'Type 3'
		else:
			Type = 'Type 4'
		print(Type)

task_classification(img)
